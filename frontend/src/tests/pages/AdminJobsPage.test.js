...
describe("AdminJobsPage tests", () => {
  ...
  test("user can submit the update course data by the quarter range job", async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <AdminJobsPage />
    ...
    expect(
      await screen.findByText("Update Courses Database by the Quarter Range"),
    ).toBeInTheDocument();

    const updateCoursesButton = screen.getByText(
      "Update Courses Database by the Quarter Range",
    );
    expect(updateCoursesButton).toBeInTheDocument();
    updateCoursesButton.click();
